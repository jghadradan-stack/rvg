...
* 🖥️ **مدیریت چند نود**: اتصال هم‌زمان به پنل‌های RVG، Marzban و 3x-ui با API Token یا نام کاربری/رمز، مانیتورینگ، مدیریت کانفیگ و ساب تجمیعی
...
| `NODE_API_TOKEN` | توکن قوی برای اجازه اتصال یک RVG مرکزی به این پنل به‌عنوان نود | — |
| `RAILWAY_PUBLIC_DOMAIN` | دامنه عمومی سرویس؛ به‌صورت خودکار توسط Railway تنظیم می‌شود | `localhost` |

## 🖥️ اتصال پنل‌های دیگر به‌عنوان نود

از منوی **نودها** می‌توانید RVG، Marzban و 3x-ui را اضافه کنید. آدرس پنل و روش ورود را وارد کنید؛ RVG قبل از ذخیره اتصال را تست می‌کند. برای اتصال توکنی RVG به RVG، روی پنل مقصد یک `NODE_API_TOKEN` قوی تنظیم و همان مقدار را در پنل مرکزی وارد کنید. برای Marzban و 3x-ui می‌توانید توکن یا نام کاربری/رمز پنل را استفاده کنید.

قابلیت‌های بخش نودها شامل مشاهده وضعیت و ترافیک، فهرست/ساخت/حذف کانفیگ‌ها از طریق API بومی پنل و خروجی ساب تجمیعی ادمین در مسیر `/api/nodes-subscription` است. اطلاعات ورود در پاسخ API و رابط مرورگر نمایش داده نمی‌شود؛ فایل وضعیت و Volume را نیز فقط در اختیار ادمین سرور قرار دهید.
